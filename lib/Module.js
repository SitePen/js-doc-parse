define([ 'dojo/_base/lang', './Value', './env', './File' ], function (lang, Value, env, File) {
	var _moduleMap = {};
	...
	Module.get = function (/**string*/ id) {
		...
		if (!_moduleMap[id]) {
			console.debug('  Module not in map');
			return Module.getByFile(new File(env.file.resolveRelativeId(id) + '.js'));
		}
	...
	Module.getByFile = function (/**File*/ file) {
		...
		if (!_moduleMap[file.moduleId]) {
			throw new Error('File ' + file.filename + ' did not register any modules we care about. WTFBBQ?');
		}
