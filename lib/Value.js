	...
	Value.prototype = {
		...
		set type(value) {
			...
			if (functionTypes[value]) {
				this.setProperty([ 'constructor' ], env.globalScope.getVariable([ 'Function' ]));
			}
		},
		...
		getProperty: function (/**Array.<string>*/ name) {
			...
			for (var variable = this, i = 0, j = name.length; i < j; ++i) {
				...
				variable = (_hasOwnProperty.call(variable.properties, name[i]) && variable.properties[name[i]]) ||
						   (variable.properties.prototype && _hasOwnProperty.call(variable.properties.prototype, name[i]) && variable.properties.prototype[name[i]]);
		...
		toArray: function () {
			if (this.type !== Value.TYPE_ARRAY) {
				throw new Error('Cannot convert non-Array to array');
			}
