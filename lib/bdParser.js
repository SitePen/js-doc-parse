define([
	...
	'./node!fs',
	'./node!util'
], function (bdParse, types, lang, env, callHandlers, File, Value, Module, fs, util) {
	types = types.symbols;

	/**
	 * @param token A node in the ASN tree.
	 * @param options context    Context for an operation.
	...
	function readTree(token, options) {
		...
		// Some binary operation
			...
			if (is(operator, types.tPunc, '[')) {
				...
				context.unshift(lhs.children, is(rhs, types.asnString) ? rhs.children : readTree(rhs));
				return context;
			}
		...
		// A return value
			...
			context.returns.push(readTree(token.children));
		}
