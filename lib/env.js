define([
	...
	'require',
	'exports'
], function (lang, Scope, File, Value, Module, standardEnv, config, console, require, exports) {
	/**
	...
	lang.mixin(exports, {
		...
		config: (function (config) {
			// Absolutise a relative base path to ensure module ID resolution always works properly
			if (config.basePath.charAt(0) !== '/') {
				// require’s baseUrl is the directory containing dojo.js, not the base directory of the app
				config.basePath = require.rawConfig.baseUrl + '/../' + config.basePath;
			}

			// Ensure the base path always ends with a slash to avoid having to think about it elsewhere
			if (config.basePath.charAt(config.basePath.length - 1) !== '/') {
				config.basePath += '/';
			}

			config.basePath = File.resolveRelativeFragments(config.basePath);
	...
	require(config.environments.map(mapModuleFragments('./env/')), function () {
		...
		globalScope.vars = (function nonEnumerableMixin(object) {
		...
		}.apply(null, [ {}, standardEnv ].concat([].slice.call(arguments, 0))));
