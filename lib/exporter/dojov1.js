define([ '../Module', '../Value', './util', '../node!util' ], function (Module, Value, util, nodeUtil) {
	/**
	 ...
	 * @param scope The type annotation for the output property, either "prototype" or "normal".
				...
				if (property.type in Value.METHOD_TYPES) {
					...
					for (i = 0; (returnValue = property.returns[i]); ++i) {
						returnsNode.createNode('return-type', {
							type: returnValue.metadata.type || returnValue.type || 'undefined'
						});
					}

					if (property.returns[0] && property.returns[0].metadata.summary) {
						propertyNode.createNode('return-description').childNodes.push(property.returns[0].metadata.summary);
					}
				}
				else {
	...
	return function (config) {
		...
		var xml = new XmlNode('javascript', { version: '1' }),
