		...
		while ((line = comment.shift()) != null) {
			...
			if (line.length && indentRe.exec(line)[0].length === keyIndent && line.length !== keyIndent) {
				...
				// Tags and type information for non-standard keys can go on the same line as the key
					...
					metadata[key].isOptional = optionalTypeRe.exec(metadata[key].type);
				}
			}
		...
		generateMetadata: function (/**Object*/ value, /**Object?*/ context) {
				...
				if (candidate && candidate.type === 'LineBlockComment') {
					...
					for (var k in metadata) {
						if (!standardKeys.hasOwnProperty(k)) {
							...
							if (value.raw.type.indexOf('Function') > -1 && _hasOwnProperty.call(value.evaluated.parameters, k)) {
								copyTo = value.evaluated.parameters[k];
							}
							...
							copyTo && mixinMetadata(copyTo.metadata, metadata[k]);
					...
					if (metadata.returns && (metadata.returns.type || metadata.returns.summary)) {
						if (!value.evaluated.returns[0]) {
							value.evaluated.returns.push(new Value({ type: metadata.returns.type || Value.TYPE_ANY }));
						}
			...
			// Object property
				...
				if (candidate && candidate.type === 'LineBlockComment' &&
					...
					metadata = processComment(candidate.value, value.raw.key.name)[value.raw.key.name];
				}
			}
